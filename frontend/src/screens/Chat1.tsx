import {
  ...
  const Chat = () => {
    ...
    const sidebarData = data?.reduce(
      (
        ...
        array.push({
          ...
          location: message.location?true:false,
          id: _id,
          profile,
          group
        });
    ...
    const {data:specifcUserChat}=useGetSpecificUserChatQuery(currentChat,{skip:currentChat?false:true});
    // console.log("Chat ~ specifcUserChat:", specifcUserChat)
          ...
          {currentChat ? (
            <>
              ...
              <div className="chat-body" ref={ulElement}>
                {
                  (()=>{
                    const {createdBy,description,group,profile,createdAt}=specifcUserChat?.[0]||{};
                ...
                {(() => {
                  ...
                  const {createdBy,description,group,profile}=specifcUserChat?.[0]||{}
                  let renderDate = "";
                  for (const [
                    index,
                    { type, message, createdAt, id,image ,location,},
                  ] of specifcUserChat?.[0]?.message?.entries() ?? []) {
